...
class test_BandOps(unittest.TestCase):
    ...
    def test_band_mapping(self):
        with self.assertRaises(AttributeError) as msg:
            ...
            self.assertEqual("BandMapping has no attribute blud. Allowed keys are blue, mask, infrared, rededge, green, red", str(msg.exception))

        self.assertDictEqual(BandMapping(), {'blue': 0, 'mask': 0, 'infrared': 0, 'rededge': 0, 'green': 0, 'red': 0})

        mydict = {'rededge': 1, 'red': 3, 'infrared': 4, 'mask': 5, }
        self.assertDictEqual(BandMapping(**mydict), {'blue': 0, 'mask': 5, 'infrared': 4, 'rededge': 1, 'green': 0, 'red': 3})

        bm = BandMapping(red=3, green=2)
        self.assertEqual(bm.allocated_bands(),[2, 3])

        bm.update(infrared=4,mask=5)
        self.assertDictEqual(bm, {'blue': 0, 'mask': 5, 'infrared': 4, 'rededge': 0, 'green': 2, 'red': 3})

        del bm['mask']
        self.assertDictEqual(bm, {'blue': 0, 'mask': 0, 'infrared': 4, 'rededge': 0, 'green': 2, 'red': 3})

    def test_calculateIndices(self):

        self.assertEqual(CalculateIndices(red=3, infrared=4, mask=5).valid_indices(), ['NDVI','PCD'])
        ci = CalculateIndices()
        self.assertDictEqual(ci.band_map,{'blue': 0, 'mask': 0, 'infrared': 0, 'rededge': 0, 'green': 0, 'red': 0} )

        ci.band_map.update({'rededge': 1, 'green': 2, 'mask': 5, 'red': 3, 'infrared': 4})
        self.assertDictEqual(ci.band_map, {'blue': 0, 'green': 2, 'infrared': 4, 'mask': 5, 'red': 3, 'rededge': 1})
        ...
        with self.assertRaises(KeyError) as msg:
            ci.calculate('NDVIa',fileImage,src_nodata=0)
